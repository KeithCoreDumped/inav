...
class MspV2Parser:
    ...
    def feed(self, data: bytes) -> tuple[list[MspFrame], list[bytes]]:
        ...
        while True:
            ...
            try:
                ...
                # No start marker found in the entire buffer
                if self.buffer:
                    raw_chunks.append(bytes(self.buffer))
                    self.buffer.clear()
                break

            # 2. Skip garbage before '$'
            if start > 0:
                raw_chunks.append(bytes(self.buffer[:start]))
                del self.buffer[:start]

            # Now self.buffer[0] == ord('$')

            # 3. Check for minimum header size (2 bytes to start checking signature)
            if len(self.buffer) < 2:
                break

            if self.buffer[1] != ord("X"):
                # Not 'X', so it's not MSPv2. 
                # Treat the '$' as raw data and continue.
                raw_chunks.append(self.buffer[0:1])
                del self.buffer[0]
                continue

            # Need at least 3 bytes to check direction
            if len(self.buffer) < 3:
                break

            direction_byte = self.buffer[2]
            if direction_byte not in (ord("<"), ord(">"), ord("!")):
                # Invalid direction. Treat '$' as raw data.
                raw_chunks.append(self.buffer[0:1])
                del self.buffer[0]
                continue

            # 4. Check for full fixed header size
            # Header: $ (1) + X (1) + Dir (1) + Flag (1) + Cmd (2) + Size (2) = 8 bytes
            ...
            # Total len = Header(8) + Payload(size) + CRC(1) = 9 + size
            total_len = 8 + size + 1
            ...
            # CRC is calculated over Flag(1) + Cmd(2) + Size(2) + Payload(size)
            # This corresponds to buffer[3 : 8+size]
...
def read_loop(
    ...
    while not stop_event.is_set():
        ...
        frames, raw_chunks = parser.feed(data)
        for chunk in raw_chunks:
            # Print non-MSP data as raw text (per user request)
